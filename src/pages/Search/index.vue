                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span
                      v-show="isOne"
                      class="iconfont"
                      :class="{ 'icon-todown': isDown, 'icon-up': isUp }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  methods: {
    ...
    attrValueInfo(attrValue) {
      this.searchParams.props.push(attrValue)
      const arr = this.searchParams.props.filter(function (
        currentValue,
        index,
        arr
      ) {
    ...
    changeOrder(flag) {
      ...
      let originDirection = this.searchParams.order.split(':')[1]
      let newOrder = ''
      if (flag === originFlag) {
        newOrder = `${originFlag}:${originDirection == 'desc' ? 'asc' : 'desc'}`
      } else {
        newOrder = `${flag}:${'desc'}`
      }
